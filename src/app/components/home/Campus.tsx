...
const locale = usePathname().split("/")[1]
  return (
    <section className='w-[100%] min-h-[250px] sm:min-h-[450px] relative flex flex-col md:flex-row justify-center items-center' data-aos="fade-up">
      ...
      <Link href={`/${locale}/news?id=campus`} className='px-4 py-3 md:px-[24px] md:py-[24px] bg-[#404B7C] shadow-md hover:text-[#404B7C] mt-6 md:mt-0 flex text-white text-[20px] md:text-[24px] rounded-[10px] items-center gap-2 ease-linear duration-200 hover:bg-white'>{t("campus")}<FaArrowRightLong /></Link>
    </section>
  )
}
