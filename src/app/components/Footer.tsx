...
const Footer = () => {
  ...
  return (
    <footer className="pt-[28px] px-3 bg_main flex flex-col items-center justify-start">
        ...
        <div className="flex flex-col items-start sm:grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-10 gap-2 w-full">
          <div className="max-w-[450px] col-span-3 w-full flex gap-4 items-start">
            <Image
              ...
              width={100}
              height={100}
              className="w-[50px] md:w-[90px] h-[50px] md:h-[90px]"
            />
            <div className="max-w-[276px] flex flex-col gap-2 text-white">
              <span className="max-w-[280px] font-bold text-white">
                {t("footer.logo")}
              </span>
              ...
              <p className="">
                {t("footer.mail")}{" "}
                <Link href="https://mail.google.com/mail/?view=cm&fs=1&to=info@tipu.uz" target="_blank">
                  info@tipu.uz
                </Link>
              </p>
              <div className="flex w-full justify-between items-center gap-2 max-w-[150px] self-start text-white text-[20px]">
                {social?.map((item, index) => {
                  return (
                    <Link target="_blank" href={item?.path} key={index}>
                      {item?.icon}
                    </Link>
                  );
                })}
          ...
          <div className="text-[16px] font-[400] col-span-2 flex flex-col gap-2 text-white">
            <Link
              href={`/${locale}/section?id=ABOUT_INSTITUTE`}
              className="text-[18px]"
            >
              {t("nav.link3")}
            </Link>
            {aboutTipi?.map((item, index) => {
              return (
                <Link href={`/${locale}/about?id=${item?.id}`} key={index}>
                  {l(locale, item?.title_uz, item?.title_ru, item?.title_en)}
                </Link>
              );
            })}
            <Link
          ...
          <div className="text-[16px] font-[400] col-span-2 flex flex-col gap-2 text-white">
            <Link
              href={`/${locale}/section?id=DEPARTMENT`}
              className="text-[18px]"
            >
              {t("nav.link5")}
            </Link>
          ...
          <iframe
            className="max-w-[350px] min-h-[200px] col-span-3 w-full rounded-2xl"
            src="https://yandex.com/map-widget/v1/?ll=69.544003%2C41.435949&mode=search&oid=193297945866&ol=biz&z=16.63"
          ></iframe>
      ...
      <Link
        href={"https://www.instagram.com/sector_soft"}
        target="_blank"
        className="self-end text-white p-3 text-[12px]"
      >
