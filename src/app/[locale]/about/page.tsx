...
const Constructor = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      setSlider(slider);
      const blog = await fetchBlog(`about-${id}${activeTab === "2" && groups ? `-2` : ""}`);
      setBlog(blog);
    };
    getData();
  }, [id, activeTab, groups]);
  ...
  return (
    <div className=" flex flex-col items-center justify-center">
     ...
     {groups && <div className="pt-5 grid grid-cols-2 gap-2 max-w-[600px] w-full">
        {items?.map((item, index) => (
          ...
          >
            <div className="relative flex items-center flex-col justify-center">
              {item?.label}
              <span className={`${item?.id === activeTab ? "w-full" : ""} ease-linear duration-200 w-0 group-hover:w-full h-[3px] bottom-0 left-0 bg-white rounded-full`}></span>
            </div>
          </button>
        ))}
      </div>}
